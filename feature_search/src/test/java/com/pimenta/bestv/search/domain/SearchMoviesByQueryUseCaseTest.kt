...
class SearchMoviesByQueryUseCaseTest {
    ...
    @Test
    fun `should return the right data when searching movies by query`() = runTest {
        whenever(movieRepository.searchMoviesByQuery(QUERY, 1)).thenReturn(WORK_PAGE)
    ...
    fun `should return an error when some exception happens`() = runTest {
        whenever(movieRepository.searchMoviesByQuery(QUERY, 1)).thenThrow(RuntimeException())
