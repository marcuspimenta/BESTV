...
class GetPopularMoviesUseCaseTest {
    ...
    @Test
    fun `should return popular movies from repository`() = runTest {
        whenever(movieRepository.getPopularMovies(PAGE)).thenReturn(PAGE_DOMAIN_MODEL)
    ...
    fun `should throw exception when repository fails`() = runTest {
        whenever(movieRepository.getPopularMovies(PAGE)).thenThrow(RuntimeException("Network error"))
