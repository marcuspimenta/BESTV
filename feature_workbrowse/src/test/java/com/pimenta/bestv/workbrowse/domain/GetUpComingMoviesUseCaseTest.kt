...
class GetUpComingMoviesUseCaseTest {
    ...
    private val useCase = GetUpComingMoviesUseCase(movieRepository)

    @Test
    fun `should return upcoming movies from repository`() = runTest {
        whenever(movieRepository.getUpComingMovies(PAGE)).thenReturn(PAGE_DOMAIN_MODEL)
    ...
    fun `should throw exception when repository fails`() = runTest {
        whenever(movieRepository.getUpComingMovies(PAGE)).thenThrow(RuntimeException("Network error"))
