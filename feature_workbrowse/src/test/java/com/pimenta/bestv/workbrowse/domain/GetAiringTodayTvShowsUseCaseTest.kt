...
class GetAiringTodayTvShowsUseCaseTest {
    ...
    private val useCase = GetAiringTodayTvShowsUseCase(tvShowRepository)

    @Test
    fun `should return airing today tv shows from repository`() = runTest {
        whenever(tvShowRepository.getAiringTodayTvShows(PAGE)).thenReturn(PAGE_DOMAIN_MODEL)
    ...
    fun `should throw exception when repository fails`() = runTest {
        whenever(tvShowRepository.getAiringTodayTvShows(PAGE)).thenThrow(RuntimeException("Network error"))
