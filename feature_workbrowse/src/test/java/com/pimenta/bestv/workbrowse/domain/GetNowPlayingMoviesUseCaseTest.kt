...
class GetNowPlayingMoviesUseCaseTest {
    ...
    private val useCase = GetNowPlayingMoviesUseCase(movieRepository)

    @Test
    fun `should return now playing movies from repository`() = runTest {
        whenever(movieRepository.getNowPlayingMovies(PAGE)).thenReturn(PAGE_DOMAIN_MODEL)
    ...
    fun `should throw exception when repository fails`() = runTest {
        whenever(movieRepository.getNowPlayingMovies(PAGE)).thenThrow(RuntimeException("Network error"))
